...
// Enum definition for queue status
enum EQueueStatus {
  CLAIMED = "CLAIMED",
  CALLED = "CALLED",
  SERVED = "SERVED",
...
export const scheduleStatusCleanup = (): void => {
  const taskFunction = async () => {
    ...
    const result = await cleanupExpiredQueuesByStatus(
      [EQueueStatus.SERVED, EQueueStatus.SKIPPED, EQueueStatus.RELEASED],
      2
    );
...
export const runJobManually = async (jobName: string): Promise<void> => {
  ...
  const jobExecutors: Record<string, () => Promise<void>> = {
    ...
    statusCleanup: async () => {
      await cleanupExpiredQueuesByStatus([EQueueStatus.SERVED, EQueueStatus.SKIPPED, EQueueStatus.RELEASED], 2);
    },
  };
