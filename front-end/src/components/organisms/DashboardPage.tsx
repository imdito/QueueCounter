...
import { useGetQueueStatus } from "@/services/queue/wrapper.service";
import { QueueStatus } from "@/services/queue/api.service";
...
const DashboardPage = () => {
  ...
  const { data: statusRes } = useGetQueueStatus();

  const stats = useMemo(() => {
    // Prefer SSE snapshot
    if (sseData) {
      const waiting = sseData.waiting ?? 0;
      const beingServed = sseData.beingServed ?? 0;
      const completed = sseData.completed ?? 0;
      const totalToday = sseData.totalToday ?? (waiting + beingServed + completed);
      // Skipped not provided directly => derive from total queues - other states
      const skipped = Math.max(totalToday - (waiting + beingServed + completed), 0);
      return { waiting, beingServed, completed, skipped };
    }
    // Fallback from queue status query
    if (statusRes?.data && 'waiting' in statusRes.data) {
      const qs = statusRes.data as QueueStatus;
      const waiting = qs.waiting ?? 0;
      const beingServed = qs.beingServed ?? 0;
      const completed = qs.completed ?? 0;
      const totalToday = qs.totalToday ?? (waiting + beingServed + completed);
      const skipped = Math.max(totalToday - (waiting + beingServed + completed), 0);
      return { waiting, beingServed, completed, skipped };
    }
    return { waiting: 0, beingServed: 0, completed: 0, skipped: 0 };
  }, [sseData, statusRes]);
          ...
          <Card className="!bg-yellow-50 !border-yellow-200">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-3xl font-bold text-yellow-900 mt-1">{stats.waiting}</h3>
              </div>
          ...
          <Card className="!bg-blue-50 !border-blue-200">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-3xl font-bold text-blue-900 mt-1">{stats.beingServed}</h3>
              </div>
          ...
          <Card className="!bg-red-50 !border-red-200">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-3xl font-bold text-red-900 mt-1">{stats.skipped}</h3>
              </div>
