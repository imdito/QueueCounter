...
export const apiGenerateTicket = async (): Promise<QueueResponse> => {
  const response = await axios.post(`${API_BASE_PATH}/generate-ticket`, {}, {
    headers: getAuthHeader()
  });
  return response.data;
};
...
export const apiCallNext = async (data: CallNextRequest): Promise<QueueResponse> => {
  const response = await axios.post(`${API_BASE_PATH}/call-next`, data, {
    headers: getAuthHeader()
  });
  return response.data;
};
...
export const apiCompleteQueue = async (id: number): Promise<QueueResponse> => {
  const response = await axios.patch(`${API_BASE_PATH}/${id}/complete`, {}, {
    headers: getAuthHeader()
  });
  return response.data;
};
...
export const apiSkipQueue = async (id: number): Promise<QueueResponse> => {
  const response = await axios.patch(`${API_BASE_PATH}/${id}/skip`, {}, {
    headers: getAuthHeader()
  });
  return response.data;
};
...
export const connectToQueueUpdates = (onMessage: (data: unknown) => void, onError?: (error: Event) => void) => {
  const eventSource = new EventSource('/api/v1/sse/queue-updates');
...
// SSE connection for counter display
export const connectToCounterDisplay = (onMessage: (data: unknown) => void, onError?: (error: Event) => void) => {
  const eventSource = new EventSource('/api/v1/sse/counter-display');
